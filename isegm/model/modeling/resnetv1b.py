...
GLUON_RESNET_TORCH = 'pretrain_resnet'
PRETRAINED_MODELS = {
    'resnet34_v1b': 'gluon_resnet34_v1b-c6d82d59.pth',
    'resnet50_v1s': 'gluon_resnet50_v1s-1762acc0.pth',
    'resnet101_v1s': 'gluon_resnet101_v1s-3b6c1f8a.pth',
    'resnet152_v1s': 'gluon_resnet152_v1s-3b6c1f8a.pth'
}
...
class ResNetV1b(nn.Module):
    ...
    def __init__(self, block, layers, classes=1000, dilated=True, deep_stem=False, stem_width=32,
        ...
        self.conv_adjust = nn.Conv2d(2048, 512, kernel_size=1)  # 添加降维操作
...
def resnet34_v1b(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        pretrained_path = os.path.join(GLUON_RESNET_TORCH, PRETRAINED_MODELS['resnet34_v1b'])
...
#         model_dict = model.state_dict()
#         filtered_orig_dict = _safe_state_dict_filtering(
#             torch.hub.load(GLUON_RESNET_TORCH_HUB, 'gluon_resnet50_v1b', pretrained=True).state_dict(),
#             model_dict.keys()
#         )
#         model_dict.update(filtered_orig_dict)
#         model.load_state_dict(model_dict)
...
def resnet50_v1s(pretrained=False, **kwargs):
    ...
    if pretrained:
        model_dict = model.state_dict()
        pretrained_path = os.path.join(GLUON_RESNET_TORCH, PRETRAINED_MODELS['resnet50_v1s'])
        if os.path.exists(pretrained_path):
            filtered_orig_dict = _safe_state_dict_filtering(
                torch.load(pretrained_path),
                model_dict.keys()
