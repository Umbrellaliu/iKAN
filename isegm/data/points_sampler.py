...
class MultiPointSampler(BasePointSampler):
    ...
    def sample_object(self, sample: DSample):                                                       # 主要作用是从给定的样本中选择一个对象，并设置相关的掩码。
        if len(sample) == 0:
    ...
    def _multi_mask_sample_points(self, selected_masks, is_negative, with_first_click=False):
        ...
        if len(points) < self.max_num_points+3:
            points.extend([(-1, -1, -1)] * (self.max_num_points+3 - len(points)))
