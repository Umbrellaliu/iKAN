...
class ISTrainer_mfp(object):
    ...
    def training(self, epoch):
        ...
        for i, batch_data in enumerate(tbar):
            ...
            loss, losses_logging, splitted_batch_data, outputs = \
                self.batch_forward(batch_data,global_step=global_step)
        ...
        for i, batch_data in enumerate(tbar):
            ...
            if self.is_master:
                ...
                for metric in self.val_metrics:
                    metric.log_states(self.sw, f'{log_prefix}Metrics/{metric.name}', global_step)
                # if self.image_dump_interval > 0 and global_step % self.image_dump_interval == 0:
                #     self.save_visualization(splitted_batch_data, outputs, global_step, prefix='val')
        ...
        if self.is_master:
            ...
            for metric in self.val_metrics:
                self.sw.add_scalar(tag=f'{log_prefix}Metrics/{metric.name}', value=metric.get_epoch_value(),
                                   global_step=epoch, disable_avg=True)
        # Save the best val_loss checkpoint 保存最佳的验证集损失模型
        current_iou = metric.get_epoch_value()
        if float(current_iou) > float(self.best_val_iou) and float(current_iou) > 0.76:
            self.best_val_iou = current_iou
            #best_epoch = epoch

            save_checkpoint(self.net, self.cfg.CHECKPOINTS_PATH, prefix='best_val',epoch=epoch, multi_gpu=self.cfg.multi_gpu)
            print(f"Saving best model to {os.path.join(self.cfg.CHECKPOINTS_PATH, f'best_val/{epoch:03d}.pth')}")

    def batch_forward(self, batch_data, validation=False, global_step=0):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            prev_mask_modulated = prev_output
            ...
            #num_iters = random.randint(1, 1)
            last_point = torch.full_like(points, -1) # 初始化上一个点
            #point_before = torch.full_like(points, -1)

            if self.use_iterloss:
                # iterloss
                vis_image_list = []
                for click_indx in range(num_iters): # numiters为1的时候不会生成新的点，为1的时候生成一个点，为2的时候生成两个

                    # v1
                    # net_input = torch.cat((image, prev_output), dim=1) \
                    #     if self.net.with_prev_mask else image
                    # v2
                    net_input = torch.cat((image, prev_output.detach(),prev_mask_modulated.detach()), dim=1) \
                        if self.net.with_prev_mask else image
                    ...
                    #print(last_point)
                    if click_indx < num_iters - 1:
                        prev_mask_modulated= modulate_prevMask(prev_output, points, self.N, self.R_max)
                    else:
                        prev_mask_modulated = prev_output
                    # last_point = last_point = get_last_point(points)
                    with torch.no_grad():
                        if self.image_dump_interval > 0 and global_step % self.image_dump_interval == 0:
                            vis_image = self.save_visualization(batch_data, output, global_step,
                                                                prefix = 'val' if validation else 'train',prev_mask_modulated=prev_mask_modulated,point_add=points)
                            vis_image_list.append(vis_image)

                if self.image_dump_interval > 0 and global_step % self.image_dump_interval == 0:
                    output_images_path = self.cfg.VIS_PATH / ('val' if validation else 'train')
                    vis_image_stacked = np.vstack(vis_image_list)
                    cv2.imwrite(str(output_images_path / f'instances_segmentation_{global_step}.jpg'),
    ...
    def add_loss(self, loss_name, total_loss, losses_logging, validation,
        ...
        if loss_weight > 0.0:
            ...
            if iterloss_step is not None:
                losses_logging[ loss_name + f'_{iterloss_step}_{iterloss_weight}' ] = loss
    ...
    def save_visualization(self, splitted_batch_data, outputs, global_step, prefix,prev_mask_modulated=None,point_add=None):
        ...
        prev_mask_modulated = prev_mask_modulated.detach().cpu().numpy() if prev_mask_modulated is not None else predicted_instance_masks

        image_blob, points= images[0], points[0]

        gt_mask = np.squeeze(gt_instance_masks[0], axis=0)
        prev_mask_modulated = np.squeeze(prev_mask_modulated[0], axis=0)
        predicted_mask = np.squeeze(predicted_instance_masks[0], axis=0)
        predicted_prob = draw_probmap(predicted_mask)
        prev_mask_modulated = draw_probmap(prev_mask_modulated)

        image = image_blob.cpu().numpy() * 255
        image = image.transpose((1, 2, 0))

        image_with_points = draw_points(image, points[:self.max_interactive_points+3], (0, 255, 0)) # 我多加了3行来存新生成的点
        image_with_points = draw_points(image_with_points, points[self.max_interactive_points+3:], (255, 0, 0))
        if point_add is not None:
            point_add = point_add.detach().cpu().numpy()
            point_add = point_add[0]
            # last_point = get_last_point(point_add)
            # if torch.any(last_point < 0) and last_point.shape[1] > 1:
            #     last_point = None
            #
            ...
            #     positive_points = last_point[last_point[:, 3] == 1]  # 正向点，label=1
            #     negative_points = last_point[last_point[:, 3] == 0]  # 负向点，label=0
            #     prev_mask_modulated = draw_points(prev_mask_modulated, positive_points[:, :2], (0, 255, 0))
            #     prev_mask_modulated = draw_points(prev_mask_modulated, negative_points[:, :2], (255, 0, 0))
            prev_mask_modulated = draw_points(prev_mask_modulated, point_add[:self.max_interactive_points+3], (0, 255, 0))
            prev_mask_modulated = draw_points(prev_mask_modulated, point_add[self.max_interactive_points+3:], (255, 0, 0))

        #gt_mask[gt_mask < 0] = 0.25
        #gt_mask = draw_probmap(gt_mask)
        #gt_mask_full = gt_mask*0.5 +image_with_points
        predicted_mask = draw_probmap(predicted_mask>0.5)
        predicted_mask = blend_mask(image_with_points,predicted_mask[:,:,0],0.5)

        gt_mask = draw_probmap(gt_mask > 0.5)
        gt_mask = blend_mask(image_with_points,gt_mask[:,:,0],0.5)
        ...
        prev_mask_modulated = add_tag(prev_mask_modulated, 'Prev mask modulated')
        image_with_points = add_tag(image_with_points, 'Image with points')
        viz_image = np.hstack((image_with_points, gt_mask,  predicted_mask,predicted_prob,prev_mask_modulated)).astype(np.uint8)
